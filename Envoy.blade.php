...
@setup
    ...
    $repo = 'git_sao@172.20.73.209:~/web/control-maquinaria.git';
    $release_dir = '/var/www/releases/control-maquinaria';
    $app_dir = '/var/www/current/control-maquinaria';
    $release_name = date('YmdHis');
    $release = $release_dir . '/' . $release_name;
    $env_dir = '/home/deployer/environments';
    $env_file = '.env.control-maquinaria';
    $env = $env_dir . '/' . $env_file;
    $storage_dir = '/var/www/storage/control-maquinaria';
@endsetup
...
@task('prepare_storage_dir')
    echo 'Setting storage directory';
...
@task('run_composer')
    ...
    cd {{ $release }};
    composer install --prefer-dist --quiet --no-scripts;
@endtask
...
@task('run_migrations')
    ...
    cd {{ $release }};
    php artisan migrate:alldb cadeco;
@endtask
